...
Func SaveEnableAcc()
	For $i = 0 To 7
		...
		IniWrite(@ScriptDir & "\Profiles\MySwitch.ini", "MySwitch", "Stay" & $i + 1, GUICtrlRead($StayTime[$i]))
...
Func ReadEnableAcc()
	For $i = 0 To 7
		...
		$iStayTime[$i] = IniRead(@ScriptDir & "\Profiles\MySwitch.ini", "MySwitch", "Stay" & $i + 1, "0")
...
Func ApplyEnableAcc()
	For $i = 0 To 7
		...
		GUICtrlSetData($StayTime[$i],$iStayTime[$i])

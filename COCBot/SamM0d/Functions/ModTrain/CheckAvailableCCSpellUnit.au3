...
Func CheckAvailableCCSpellUnit()
	...
	While 1
		...
		For $i = 0 To $iSlotCount
			...
			Else
				SetLog("Error detect quantity no. On CC Spell: " & NameOfTroop(Eval("enum" & $aiSpellsInfo[$i][0]) + $eLSpell, $aiSpellsInfo[$i][1]),$COLOR_RED)
				ExitLoop
			EndIf
		Next
		...
		For $i = 0 To UBound($MySpells) - 1
			...
			If $itempTotal > 0 Then
				SetLog(" - No. of Available CC Spells - " & NameOfTroop(Eval("enum" & $MySpells[$i][0]) + $eLSpell,  Eval("curCCSpell" & $MySpells[$i][0])) & ": " &  Eval("curCCSpell" & $MySpells[$i][0]), (Eval("enum" & $MySpells[$i][0]) > $iDarkFixSpell ? $COLOR_DARKELIXIR : $COLOR_ELIXIR))
					...
					If $aiSpellsInfo[$i][1] <> 0 Then
						...
						If $iUnitToRemove > 0 Then
							If $aiSpellsInfo[$i][1] > $iUnitToRemove Then
								SetLog("Remove " & NameOfTroop(Eval("enum" & $aiSpellsInfo[$i][0]) + $eLSpell,  $aiSpellsInfo[$i][1]) & " at slot: " & $aiSpellsInfo[$i][2] & ", unit to remove: " & $iUnitToRemove, $COLOR_ACTION)
								RemoveCCSpells($aiSpellsInfo[$i][2]-1, $iUnitToRemove, $iOffsetSlot)
								$iUnitToRemove = 0
								Assign("RemoveUnitOfcurCCSpell" & $aiSpellsInfo[$i][0], $iUnitToRemove)
							Else
								SetLog("Remove " & NameOfTroop(Eval("enum" & $aiSpellsInfo[$i][0]) + $eLSpell,  $aiSpellsInfo[$i][1]) & " at slot: " & $aiSpellsInfo[$i][2] & ", unit to remove: " & $aiSpellsInfo[$i][1], $COLOR_ACTION)

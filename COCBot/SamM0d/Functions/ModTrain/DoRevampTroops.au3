...
Func DoRevampTroops($bDoPreTrain = False)
	...
	For $i = 0 To UBound($tempTroops) - 1
		If Eval("Dif" & $tempTroops[$i][0]) > 0 Then
			...
			;SetLog("Prepare for train number Of " & NameOfTroop(Eval("e" & $tempTroops[$i][0]), Eval("Dif" & $tempTroops[$i][0])) & " x" & Eval("Dif" & $tempTroops[$i][0]),$COLOR_ACTION)
	...
	If $bReVampFlag Then
		If gotoTrainTroops() = False Then Return
			...
			For $i = 0 To UBound($tempTroops) - 1
				...
				If $iOnQQty > 0 Then
					SetLog($CustomTrain_MSG_5 & " " & NameOfTroop(Eval("e" & $tempTroops[$i][0]), $iOnQQty) & " x" & $iOnQQty,$COLOR_ACTION)
				EndIf
			Next
				...
				If $Troop4Add > 0 And $iRemainTroopsCapacity > 0 Then
					...
					If $ichkEnableMySwitch = 0 Then
						If $tempTroops[$i][4] = 0 Then
							$iCost = CostTroopsOcr($g_iTroopButtonX, $g_iTroopButtonY)
					...
					If ($Troop4Add * $iCost) > $iBuildCost Then
						...
						Setlog($CustomTrain_MSG_8 & " " & (Eval("e" & $tempTroops[$i][0]) > $iDarkFixTroop ? $CustomTrain_MSG_DarkElixir : $CustomTrain_MSG_Elixir) & " " & $CustomTrain_MSG_9 & " " & NameOfTroop(Eval("e" & $tempTroops[$i][0]),0), $COLOR_ERROR)
					...
					EndIf
					; use eval and not $i to compare because of maybe after array sort $tempTroops
					SetLog($CustomTrain_MSG_6 & " " & NameOfTroop(Eval("e" & $tempTroops[$i][0]),$Troop4Add) & " x" & $Troop4Add & " " & $CustomTrain_MSG_7 & " " & (Eval("e" & $tempTroops[$i][0]) > $iDarkFixTroop ? $CustomTrain_MSG_DarkElixir : $CustomTrain_MSG_Elixir) & " : " & ($Troop4Add * $iCost),(Eval("e" & $tempTroops[$i][0]) > 12 ? $COLOR_DARKELIXIR : $COLOR_ELIXIR))
					...
					Else
						...
						SetLog("troops above cannot fit to max capicity, reduce to train " & NameOfTroop(Eval("e" & $tempTroops[$i][0]),$iReduceCap) & " x" & $iReduceCap,$COLOR_ERROR)
...
;~ 						CheckNeedSwipe($eArch)
						SetLog("still got remain capacity, so train " & NameOfTroop(Eval("eArch"),$fixRemain) & " x" & $fixRemain & " to fit it.",$COLOR_ERROR)
...
Func CostTroopsOcr($iButtonX = 0, $iButtonY = 0)
    Local $ixLowest = 0
	Local $ixBiggest = 0
	Local $iyBiggest = 0
	Local $iResult = 0
	;X -X =B
	For $b = 0 To 101
	;is 0xD3D3CB in x cord - $b then
	If _ColorCheck(_GetPixelColor($iButtonX - $b, $iButtonY, True), Hex(0xD3D3CB, 6), 5) = True Then
		$ixLowest = $iButtonX - $b
		Setlog($ixLowest)
	exitloop
	endif
	...
	;X +X =C
	For $c = 0 To 101
	;is 0xD3D3CB in x cord + $c then 
	If _ColorCheck(_GetPixelColor($iButtonX + $c, $iButtonY, True), Hex(0xD3D3CB, 6), 5) = True Then
		$ixBiggest = $iButtonX + $c
		Setlog($ixBiggest)
	...
	;Y +Y =A
	For $a = 0 To 101
	;is 0xD3D3CB in y cord + $a then 
	If _ColorCheck(_GetPixelColor($iButtonX, $iButtonY + $a, True), Hex(0xD3D3CB, 6), 5) = True Then 
		$iyBiggest = $iButtonY + $a
		Setlog($iyBiggest)
	exitloop
	endif
	Next
		
	$iResult = getMyOcr(0,$iButtonX - 55,$iButtonY + 26, 68, 18,"troopcost",True,False,True)

...
Func TNRQT($TroopsQueueFullOnly = False, $TroopsQueueTrain = True, $TroopsQueueRemove = True, $OpenTabForQueue = False)
	...
	If $TroopsQueueTrain = True Then
		TrainUsingWhatToTrainSam($TroopsToTrain)
		TrainUsingWhatToTrainSam($TroopsToTrain, True)
	EndIf
...
Func RemoveQueueTroops($rWTT)
	...
	For $i = 0 To (UBound($rWTT) - 1)
		...
		If $iIndex >= $eBarb And $iIndex <= $eIceG Then
			...
			$PositionQueue = FindImageInPlace($rWTT[$i][0], $g_sImgArmyOverviewTroopQueued & $rWTT[$i][0], $sRegion);$g_sImgArmyOverviewTroopQueued & $rWTT[$i][0] becomes \imgxml\ArmyOverview\TroopQueued\Arch
						If StringInStr($PositionQueue, ",") > 0 Then
							$PositionQueue = StringSplit($PositionQueue, ",", $STR_NOCOUNT)
						EndIf
		ElseIf $iIndex >= $eLSpell And $iIndex <= $eBtSpell Then
			...
			$PositionQueue = FindImageInPlace($rWTT[$i][0], $g_sImgArmyOverviewSpellQueued & $rWTT[$i][0], $sRegion);$g_sImgArmyOverviewSpellQueued & $rWTT[$i][0] becomes \imgxml\ArmyOverview\SpellQueued\CSpell
						If StringInStr($PositionQueue, ",") > 0 Then
							$PositionQueue = StringSplit($PositionQueue, ",", $STR_NOCOUNT)
						EndIf
		EndIf
...
Func BCheckIsFullQueuedAndNotFullArmy()
	...
	If UBound($aArmyCamp) = 3 And $aArmyCamp[2] < 0 Then
		If _ColorCheck(_GetPixelColor($CheckTroop[0], $CheckTroop[1], True), Hex($CheckTroop[2], 6), $CheckTroop[3]) Then
			...
			If IsArmyWindow(False, $TrainTroopsTAB) Then SimpleTTrain("Arch", $ArchToMake, $g_iTrainClickDelay) ; PureClick($TrainArch[0], $TrainArch[1], $ArchToMake, 500)
...
Func SimpleTTrain($iIndex, $iQuantityTmp = 0, $iSleep = 400)
	Local $bIsBrewSpell = False
	Local $bTroop = False
	Local $sTroopSelected
	Local $iQuantity = Int($iQuantityTmp)
	
	For $i = 0 To UBound($MyTroops) - 1
	    If not StringCompare($MyTroops[$i][0], $iIndex) = 0 Then
			ContinueLoop
		Else 
			$sTroopSelected = $MyTroops[$i][0]
			$bTroop = True
			ExitLoop
		EndIf
	Next
	
	If $bTroop = False Then
		For $ii = 0 To UBound($MySpells) - 1
			If not StringCompare($MySpells[$ii][5], $iIndex) = 0 Then
				ContinueLoop
			Else
				$sTroopSelected = $MySpells[$ii][0]
				$bIsBrewSpell = True
				ExitLoop
			EndIf
		Next
	...
	If LocateTroopButton($sTroopSelected, $bIsBrewSpell) Then
		If _Sleep(500) Then Return
		If MyTrainClick($g_iTroopButtonX, $g_iTroopButtonY, $iQuantity, $iSleep, "#TT01") Then
			SetLog("Training" & $iIndex, $COLOR_GREEN)
		Else
			SetLog("Cannot find button " & $iIndex & " for click " & $iQuantity & " Times", $COLOR_ERROR)
		EndIf
	EndIf
EndFunc
...
Func DragNDropTrain($rWTT, $reloop)
	...
	If $TroopCamp[0] <= $TroopCamp[1] Or $TroopCamp1[0] <= $TroopCamp1[1] Then ; Check Total Camp
		...
		TrainUsingWhatToTrainSam($rWTT)
		TrainUsingWhatToTrainSam($rWTT, True)
	EndIf
...
Func RemoveExtraTroopsDND() ; Official Func Modified By Ahsan Iqbal
	...
	Local $toRemove = WhatToTrainSam(True, False)
	...
	If TotalSpellsToBrewInGUISam() > 0 Then
...
Func WhatToTrainSam($ReturnExtraTroopsOnly = False, $bSetLog = True)
	OpenArmyTab(True, "WhatToTrainSam()")
	...
	If $g_bIsFullArmywithHeroesAndSpells And Not $ReturnExtraTroopsOnly Then
		...
		For $i = 0 To UBound($MyTroops) - 1
			If $MyTroops[$i][3] > 0 Then
				$ToReturn[UBound($ToReturn) - 1][0] = $MyTroops[$i][0]
				$ToReturn[UBound($ToReturn) - 1][1] = $MyTroops[$i][3]
		...
		For $i = 0 To UBound($MySpells) - 1
			Local $BrewIndex = $i
			If TotalSpellsToBrewInGUISam() = 0 Then ExitLoop
			If $MySpells[$i][3] > 0 Then
				If HowManyTimesWillBeUsed($MySpells[$i][5]) > 0 Then
					$ToReturn[UBound($ToReturn) - 1][0] = $MySpells[$i][5]
					$ToReturn[UBound($ToReturn) - 1][1] = $MySpells[$i][3]
					ReDim $ToReturn[UBound($ToReturn) + 1][2]
				Else
					getArmySpells(False, False, False, False)
					If $MySpells[$i][3] - $g_aiCurrentSpells[$BrewIndex] > 0 Then
						$ToReturn[UBound($ToReturn) - 1][0] = $MySpells[$i][5]
						$ToReturn[UBound($ToReturn) - 1][1] = $MySpells[$i][3] - $g_aiCurrentSpells[$BrewIndex]
						ReDim $ToReturn[UBound($ToReturn) + 1][2]
					Else
						$ToReturn[UBound($ToReturn) - 1][0] = $MySpells[$i][5]
	...
	Switch $ReturnExtraTroopsOnly
		Case False
			...
			For $ii = 0 To UBound($MyTroops) - 1
				Local $troopIndex = $ii 
				If $MyTroops[$ii][3] > 0 Then
					$ToReturn[UBound($ToReturn) - 1][0] = $MyTroops[$ii][0]
					$ToReturn[UBound($ToReturn) - 1][1] = $MyTroops[$ii][3] - $g_aiCurrentTroops[$troopIndex]
					ReDim $ToReturn[UBound($ToReturn) + 1][2]
				EndIf
			Next
		...
		Case Else
			...
			For $ii = 0 To UBound($MyTroops) - 1
				Local $troopIndex = $ii 
				If $MyTroops[$ii][3] > 0 Then
					If $MyTroops[$ii][3] - $g_aiCurrentTroops[$troopIndex] < 0 Then
						$ToReturn[UBound($ToReturn) - 1][0] = $MyTroops[$ii][0]
						$ToReturn[UBound($ToReturn) - 1][1] = Abs($MyTroops[$ii][3] - $g_aiCurrentTroops[$troopIndex])
						ReDim $ToReturn[UBound($ToReturn) + 1][2]
					EndIf
				EndIf
			Next
			...
			For $i = 0 To UBound($MySpells) - 1
				...
				If $MySpells[$i][3] > 0 Then
					If $MySpells[$i][3] - $g_aiCurrentSpells[$BrewIndex] < 0 Then
						$ToReturn[UBound($ToReturn) - 1][0] = $MySpells[$i][5]
						$ToReturn[UBound($ToReturn) - 1][1] = Abs($MySpells[$i][3] - $g_aiCurrentSpells[$BrewIndex])
						ReDim $ToReturn[UBound($ToReturn) + 1][2]
					EndIf
				EndIf
			Next
...
Func TrainUsingWhatToTrainSam($rWTT, $bSpellsOnly = False)
	If Not $g_bRunState Then Return
	;	_ArrayDisplay($rWTT)
	...
	If Not $bSpellsOnly Then
		If Not OpenTroopsTab(True, "TrainUsingWhatToTrainSam()") Then Return
	Else
		If Not OpenSpellsTab(True, "TrainUsingWhatToTrainSam()") Then Return
	EndIf
	
    If Not $g_bIsFullArmywithHeroesAndSpells Then 
		For $i = 0 To (UBound($rWTT) - 1)
			SimpleTTrain($rWTT[$i][0], $rWTT[$i][1])
		Next
	EndIf
	Return
			...
			For $i = 0 To (UBound($rWTT) - 1)
				...
				If $rWTT[$i][1] > 0 Then ; If Count to Train Was Higher Than ZERO
					If IsSpellToBrew($rWTT[$i][0]) Then
						If $bSpellsOnly Then SimpleTTrain($rWTT[$i][0], $rWTT[$i][1])
						ContinueLoop
					Else
						If $bSpellsOnly Then ContinueLoop
					EndIf
					Local $NeededSpace = CalcNeededSpaceSam($rWTT[$i][0], $rWTT[$i][1])
					...
					If $NeededSpace <= $LeftSpace Then ; If Needed Space was Equal Or Lower Than Left Space
						...
						Local $iTroopIndex = TroopIndexLookup($rWTT[$i][0], "TrainUsingWhatToTrainSam#1")
						...
						If CheckValuesCost($rWTT[$i][0], $rWTT[$i][1]) Then
							...
							SimpleTTrain($rWTT[$i][0], $rWTT[$i][1], $g_iTrainClickDelay)
						Else
					...
					Else ; If Needed Space was Higher Than Left Space
						...
						Local $CanAdd = True
						Do
							$NeededSpace = CalcNeededSpaceSam($rWTT[$i][0], $CountToTrain)
						...
						Local $iTroopIndex = TroopIndexLookup($rWTT[$i][0], "TrainUsingWhatToTrainSam#2")
						...
						If CheckValuesCost($rWTT[$i][0], $CountToTrain) Then
							...
							SimpleTTrain($rWTT[$i][0], $CountToTrain, $g_iTrainClickDelay)
						Else
					...
					EndIf
				EndIf
				If _Sleep($DELAYRESPOND) Then Return ; add 5ms delay to catch SimpleTTrain errors, and force return to back to main loop
			Next
		Case True
			For $i = 0 To (UBound($rWTT) - 1)
				If Not $g_bRunState Then Return
				If $rWTT[$i][1] > 0 Then ; If Count to Train Was Higher Than ZERO
					...
					If $NeededSpace <= $LeftSpace Then ; If Needed Space was Equal Or Lower Than Left Space
						...
						Local $iTroopIndex = TroopIndexLookup($rWTT[$i][0], "TrainUsingWhatToTrainSam#3")
					...
					Else ; If Needed Space was Higher Than Left Space
						...
						Local $iTroopIndex = TroopIndexLookup($rWTT[$i][0], "TrainUsingWhatToTrainSam#4")
	...
	Return True
EndFunc   ;==>TrainUsingWhatToTrainSam

Func CalcNeededSpaceSam($Troop, $Quantity)
	If Not $g_bRunState Then Return -1
	Local $iDbg = 1
	For $i = 0 To UBound($MyTroops) - 1
	    If not StringCompare($MyTroops[$i][0], $Troop) = 0 Then
			ContinueLoop
		EndIf
		If $iDbg = 1 Then Setlog(Number($MyTroops[$i][2] * $Quantity))
		Return Number($MyTroops[$i][2] * $Quantity)
	Next

	For $ii = 0 To UBound($MySpells) - 1
	    If not StringCompare($MySpells[$ii][5], $Troop) = 0 Then
			ContinueLoop
		EndIf
		If $iDbg = 1 Then Setlog(Number($MySpells[$ii][2] * $Quantity))
		Return Number($MySpells[$ii][2] * $Quantity)
	Next
	Return -1
EndFunc   ;==>CalcNeededSpaceSam

Func TotalSpellsToBrewInGUISam()
	Local $iTotalSpellsInGUI = 0
	If $g_iTotalSpellValue = 0 Then Return $iTotalSpellsInGUI
	If Not $g_bRunState Then Return
	For $i = 0 To UBound($MySpells) - 1
		$iTotalSpellsInGUI += $MySpells[$i][3] * $MySpells[$i][2]
	Next
	Return $iTotalSpellsInGUI
EndFunc   ;==>TotalSpellsToBrewInGUISam

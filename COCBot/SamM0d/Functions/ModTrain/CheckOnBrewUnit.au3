...
Func CheckOnBrewUnit($hHBitmap)
	...
	For $i = 10 To 0 Step -1
		...
		Else
			...
			If $iQty <> 0 And $sObjectname <> "" Then
				...
				EndIf
			Else
				SetLog("Error detect quantity no. On Spell: " & NameOfTroop(Eval("enum" & $sObjectname) + $eLSpell , $iQty),$COLOR_RED)
				ExitLoop
			EndIf
		EndIf
	Next
	...
	For $i = 0 To UBound($MySpells) - 1
		...
		If Eval("OnT" & $MySpells[$i][0] & "Spell") > 0 Then
			SetLog(" - No. of On Brew " & NameOfTroop(Eval("enum" & $MySpells[$i][0]) + $eLSpell,  Eval("OnT" & $MySpells[$i][0] & "Spell")) & ": " &  Eval("OnT" & $MySpells[$i][0] & "Spell"), (Eval("enum" & $MySpells[$i][0]) > $iDarkFixSpell ? $COLOR_DARKELIXIR : $COLOR_ELIXIR))
			$bGotOnBrewFlag = True
		EndIf
		If $MySpells[$i][3] < $itempTotal Then
			If $ichkEnableDeleteExcessSpells = 1 Then
				SetLog("Error: " & NameOfTroop(Eval("enum" & $MySpells[$i][0] + $eLSpell),  Eval("OnT" & $MySpells[$i][0] & "Spell")) & " need " & $MySpells[$i][3] & " only, and i made " & $itempTotal)
			...
			If $aiSpellInfo[$i][1] <> 0 And $aiSpellInfo[$i][3] = False Then
				...
				If $iUnitToRemove > 0 Then
					If $aiSpellInfo[$i][1] > $iUnitToRemove Then
						SetLog("Remove " & NameOfTroop(Eval("enum" & $aiSpellInfo[$i][0]) + $eLSpell,  $aiSpellInfo[$i][1]) & " at slot: " & $aiSpellInfo[$i][2] & ", unit to remove: " & $iUnitToRemove, $COLOR_ACTION)
						RemoveTrainTroops($aiSpellInfo[$i][2]-1, $iUnitToRemove)
						$iUnitToRemove = 0
						Assign("RemoveSpellUnitOfOnT" & $aiSpellInfo[$i][0], $iUnitToRemove)
					Else
						SetLog("Remove " & NameOfTroop(Eval("enum" & $aiSpellInfo[$i][0]) + $eLSpell,  $aiSpellInfo[$i][1]) & " at slot: " & $aiSpellInfo[$i][2] & ", unit to remove: " & $aiSpellInfo[$i][1], $COLOR_ACTION)
	...
	Else
		...
		For $i = 0 To UBound($MySpells) - 1
			...
			If $itempTotal > 0 Then
				SetLog(" - No. of On Queue " & NameOfTroop(Eval("enum" & $MySpells[$i][0]) + $eLSpell,  Eval("OnQ" & $MySpells[$i][0] & "Spell")) & ": " &  Eval("OnQ" & $MySpells[$i][0] & "Spell"), (Eval("enum" & $MySpells[$i][0]) > $iDarkFixSpell ? $COLOR_DARKELIXIR : $COLOR_ELIXIR))
				$bGotOnQueueFlag = True
				If Eval("ichkPre" & $MySpells[$i][0]) = 1 Then
					If $MySpells[$i][3] < $itempTotal Then
						If $ichkEnableDeleteExcessSpells = 1 Then
							SetLog("Error: " & NameOfTroop(Eval("enum" & $MySpells[$i][0]) + $eLSpell,  Eval("OnQ" & $MySpells[$i][0] & "Spell")) & " need " & $MySpells[$i][3] & " only, and i made " & $itempTotal)
				...
				Else
					SetLog("Error: " & NameOfTroop(Eval("enum" & $MySpells[$i][0]) + $eLSpell,  Eval("OnQ" & $MySpells[$i][0] & "Spell")) & " not needed to pre brew, remove all.")
				...
				For $i = 0 To UBound($aiSpellInfo) - 1
					If $aiSpellInfo[$i][3] = True Then
						If $aiSpellInfo[$i][0] <> $tempSpells[0][0] Then
							SetLog("Pre-Brew Spell first slot: " & NameOfTroop(Eval("enum" & $aiSpellInfo[$i][0])+ $eLSpell, $aiSpellInfo[$i][1]), $COLOR_ERROR)
							SetLog("My first order spells: " & NameOfTroop(Eval("enum" & $tempSpells[0][0])+ $eLSpell, $tempSpells[0][3]), $COLOR_ERROR)
						...
						Else
							If $aiSpellInfo[$i][1] < $tempSpells[0][3] Then
								SetLog("Pre-Brew Spell first slot: " & NameOfTroop(Eval("enum" & $aiSpellInfo[$i][0])+ $eLSpell, $aiSpellInfo[$i][1]) & " - Units: " & $aiSpellInfo[$i][1], $COLOR_ERROR)
								SetLog("My first order spells: " & NameOfTroop(Eval("enum" & $tempSpells[0][0])+ $eLSpell, $tempSpells[0][3]) & " - Units: " & $tempSpells[0][3], $COLOR_ERROR)
	...
	If $g_abAttackTypeEnable[$DB] = True And $g_abSearchSpellsWaitEnable[$DB] = True Then
		For $i = $enumLightning To $enumSkeleton
			If Eval("Cur" & $MySpells[$i][0] & "Spell") < $MySpells[$i][3] Then
				SETLOG(" Dead Base - Waiting " & NameOfTroop($i+ $eLSpell, $MySpells[$i][3] - Eval("Cur" & $MySpells[$i][0] & "Spell")) & _
	...
	If $g_abAttackTypeEnable[$LB] = True And $g_abSearchSpellsWaitEnable[$LB] = True Then
		For $i = $enumLightning To $enumSkeleton
			If Eval("Cur" & $MySpells[$i][0] & "Spell") < $MySpells[$i][3] Then
				SETLOG(" Live Base - Waiting " & NameOfTroop($i+ $eLSpell+ $eLSpell, $MySpells[$i][3] - Eval("Cur" & $MySpells[$i][0] & "Spell")) & _
				" to brew finish before start next attack.", $COLOR_ACTION)
			EndIf
		Next
	EndIf

...
Func BuilderBaseSelectCorrectScript(ByRef $AvailableTroops)
	...
	For $i = 0 To $CampsQuantities - 1
		If StringCompare($NewAvailableTroops[$i][0], $aCamps[$i]) <> 0 Then
			...
			Local $aAttackBar = _ImageSearchXMLBoludoz($g_sBundleAttackBarBB, $g_aBundleAttackBarSwitchBBParms[0], $g_aBundleAttackBarSwitchBBParms[1], $g_aBundleAttackBarSwitchBBParms[2], False)

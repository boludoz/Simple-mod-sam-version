...
Func BuilderBaseParseAttackCSV($AvailableTroops, $DeployPoints, $DeployBestPoints, $bDebug = False)
	...
	; [x][0] = Troops Name , [x][1] = X-axis , [x][2] - Quantities
	...
	If FileExists($FileNamePath) Then
						...
						If $command = 1 Then
							...
							Switch $i
								Case $g_iAirDefense
									...
									Setlog("Detected Air defenses: " & UBound($g_aCrusherPos))
									ExitLoop
								Case $g_iCrusher
									$g_aCrusherPos = BuilderBaseBuildingsDetection($g_iCrusher)
									Setlog("Detected Crusher: " & UBound($g_aCrusherPos))
									ExitLoop
								Case $g_iGuardPost
									$g_aGuardPostPos = BuilderBaseBuildingsDetection($g_iGuardPost)
									Setlog("Detected GuardPost: " & UBound($g_aCrusherPos))
									ExitLoop
								Case $g_iCannon
									$g_aCannonPos = BuilderBaseBuildingsDetection($g_iCannon)
									Setlog("Detected Cannon: " & UBound($g_aCrusherPos))
									ExitLoop
								Case $g_iBuilderHall
				...
				Case "DROP"
					...
					For $i = 0 To UBound($aAvailableTroops_NXQ) - 1
						If $sTroopName = $aAvailableTroops_NXQ[$i][0] Then
							$iTotalQtyByTroop += $aAvailableTroops_NXQ[$i][2]
						EndIf
					Next
					...
					If Not VerifySlotTroop($sTroopName, $aSlot_XY, $iQtyOfSelectedSlot, $iSlotNumber, $aAvailableTroops_NXQ) Then
						TriggerMachineAbility($bIfMachineHasAbility, $aMachineSlot_XYA) ;Click On Ability Before Skipping The Loop
						ContinueLoop
					EndIf
						...
						While $iTroopsDropped < $iQtyToDrop
							For $i = 0 To UBound($aCSVParsedDeployPoint_XY) - 1
								...
								If ($iTroopsDropped < $iQtyToDrop Or $sQty = "ALL") Then ; Check That Drop Troops Don't Get Exceeded By CSV DROP Quantity OR Drop all troops if Qunanity is ALL
									...
									If IsMachineDepoloyed($sTroopName, $aSlot_XY, $iSlotNumber, $bIfMachineWasDeployed, $bIfMachineHasAbility, $aMachineSlot_XYA) Then ExitLoop (2)
									; Just in case
							...
							If $g_aBuilderHallPos[0][0] <> Null Then
								...
								If $Point2Deploy[0] <> "" Then
									For $i = 0 To $iQtyToDrop - 1
										...
										$aAvailableTroops_NXQ[$iSlotNumber][2] = $iQtyOfSelectedSlot
										; If is the Machine exist and deployed
										If IsMachineDepoloyed($sTroopName, $aSlot_XY, $iSlotNumber, $bIfMachineWasDeployed, $bIfMachineHasAbility, $aMachineSlot_XYA) Then ExitLoop
					...
					; A log user
					SetLog(" - " & $aAvailableTroops_NXQ[$iSlotNumber][2] & "x/" & $iTotalQtyByTroop & "x " & FullNametroops($aAvailableTroops_NXQ[$iSlotNumber][0]) & " at slot " & $iSlotNumber + 1, $COLOR_WARNING)

					For $i = 0 To Int($SleepAfter / 50)
						; Machine Ability
						TriggerMachineAbility($bIfMachineHasAbility, $aMachineSlot_XYA)
		...
		Local $aAvailableTroops_NXQ = BuilderBaseAttackBar($bIfMachineWasDeployed, $bIfMachineHasAbility, $aMachineSlot_XYA)
		...
		BattleIsOver($bIfMachineHasAbility, $aMachineSlot_XYA)
...
Func SortPoints(ByRef $aSelectedDropSidePoints_XY, $bDebug)
...
Func VerifySlotTroop($sTroopName, ByRef $aSlot_XY, ByRef $iQtyOfSelectedSlot, ByRef $iSlotNumber, $aAvailableTroops_NXQ)
	...
	For $i = 0 To UBound($aAvailableTroops_NXQ) - 1
		...
		If $sTroopName = $aAvailableTroops_NXQ[$i][0] And $iSlotNumber <> $i Then
			...
			$iQtyOfSelectedSlot = $aAvailableTroops_NXQ[$i][2]
	...
	If $iQtyOfSelectedSlot = 0 Or $iSlotX = 0 Then
...
Func DeployTroopBB($sTroopName, $aSlot_XY, $Point2Deploy, $iQtyToDrop)
	SetDebugLog("[" & $aSlot_XY[0] & "] - Deploying " & $iQtyToDrop & " " & FullNametroops($sTroopName) & " At " & $Point2Deploy[0] & " x " & $Point2Deploy[1], $COLOR_INFO)
	; Protect the Attack Bar
	If $Point2Deploy[1] > 535 Then $Point2Deploy[1] = 535
...
Func TriggerMachineAbility(ByRef $bIfMachineHasAbility, $aMachineSlot_XYA, $isFirstTime = False)
	If Not $bIfMachineHasAbility Then Return
...
Func IsMachineDepoloyed($sTroopName, $aSlot_XY, $iSlotNumber, ByRef $bIfMachineWasDeployed, ByRef $bIfMachineHasAbility, ByRef $aMachineSlot_XYA)
	If $bIfMachineWasDeployed Then Return ;If already deployed no need to check again
	If Not $g_bRunState Then Return
	If $sTroopName = "Machine" Then
		$bIfMachineWasDeployed = True

...
Func WallsUpgradeBB()
	...
	If isOnBuilderBase() Then ; Double Check to see if Bot is on builder base
		...
		If $g_iFreeBuilderCountBB > 0 And $g_bChkBBUpgradeWalls = True And Number($g_aiCurrentLootBB[$eLootGoldBB]) > $g_aiWallBBInfoPerLevel[$BBWallLevel + 1][1] Then
			...
			Local $WallsBBNXY = _ImageSearchBundlesMyBot($g_sBundleWallsBB, $g_aBundleWallsBBParms[0], $g_aBundleWallsBBParms[1], $g_aBundleWallsBBParms[2], $g_bDebugBBattack, True, "10", $BBWallLevel)
